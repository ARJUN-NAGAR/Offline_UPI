...
public class HybridCryptoService {
    ...
    private static final int RSA_TRANSCODER_KEY_BYTES = 256; // for 2048-bit RSA
    ...
    public PaymentInstruction decrypt(String base64Ciphertext) throws Exception {
        ...
        if (all.length < RSA_TRANSCODER_KEY_BYTES + GCM_IV_BYTES + GCM_TAG_BITS / 8) {
            throw new IllegalArgumentException("Ciphertext too short");
        }

        // Unpack
        byte[] encryptedAesKey = new byte[RSA_TRANSCODER_KEY_BYTES];
        byte[] iv = new byte[GCM_IV_BYTES];
        byte[] aesCiphertext = new byte[all.length - RSA_TRANSCODER_KEY_BYTES - GCM_IV_BYTES];
